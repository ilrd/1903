    ...
    try:
        ...
        if message.chat.type == 'private':
            if READ_SYMPTOM:
                ...
                # for i in range(5):
                #     if i < len(doc_pred):
                #         doc_predi = doc_pred[i]
                #         out.append(doc_predi[0])
                #         out.append(doc_predi[1])
                #     else:
                #         out.append('')
                #         out.append('')
                # bot.send_message(message.chat.id,
                #                  SYMPTOM_PRED.format(*pred, *out))
                ...
                if text not in SPEC_LIST:
                    ...
                    bot.send_message(message.chat.id,
                                     DOC_FIND.format(text, doc_pred[0][0], doc_pred[0][1], doc_pred[1][0],
                                                     doc_pred[1][1],
                                                     doc_pred[2][0], doc_pred[2][1], doc_pred[3][0], doc_pred[3][1],
